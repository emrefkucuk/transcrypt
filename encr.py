...
def example_file_transfer():
    """
    Example demonstrating how to use the secure file transfer functions.
    This is just for demonstration and testing - not for production use.
    """
    # Sample file data (in a real application, this would be read from a file)
    sample_file = b"This is a sample file content for encryption and decryption testing."
    
    print("Starting secure file transfer demonstration...")
    
    # 1. Generate RSA key pair for the receiver
    private_key, public_key = generate_rsa_key_pair()
    print("Generated RSA key pair for receiver")
    
    # 2. Sender processes the file for secure transfer
    transfer_package = process_file_for_sending(sample_file, public_key)
    print("File encrypted and prepared for transfer")
    
    # 3. Receiver processes the received file
    received_package = process_received_file(transfer_package, private_key)
    print("File received and decrypted")
    
    # 4. Check results
    if received_package['integrity_verified']:
        print("File integrity verified: The file was not tampered with during transfer")
    else:
        print("Warning: File integrity check failed!")
        
    print("Original file:", sample_file)
    print("Decrypted file:", received_package['decrypted_data'])
    print("Transfer successful:", sample_file == received_package['decrypted_data'])
